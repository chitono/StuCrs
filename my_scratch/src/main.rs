...
fn main() {
    ...
    */
    ...
    for epoch in 0..max_epoch {
        ...
        let y = model.call(&x_train.rv());
        println!("y ={:?}", y.data().backend_type());

        let mut loss = F::softmax_cross_entropy_simple(&y, &y_train.rv());
        println!("loss ={:?}", loss.data().backend_type());
        ...
        println!("epoch = {:?}, train_loss = {:?}", epoch + 1, loss.data(),);
