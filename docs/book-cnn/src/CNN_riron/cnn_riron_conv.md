...
フィルターというものを用意し、それを画像に処理させます。この処理の流れは　\\(5 = 0\cdot 9 + 1\cdot 5 + 1\cdot 0 + 0\cdot 1\\)という計算になります。そして、このフィルターをスライドさせ、同じように計算した値を移動した場所の値とします。この畳み込みの処理にはいくつかの設定があります。それは **ストライド** と **パディング** です。

## ストライド
ストライドは、フィルターが一度にどれくらいスライドするかを指定するものです。画像は2次元なので、縦と横の二つの値を設定します。いくつか例を見てみましょう。例えば縦、横どちらとも一つずつスライドする場合、(1,1)と指定します。すると下の図のようになります。


**(1,1)の場合**
![alt text](<スクリーンショット 2026-04-13 175722.png>)

---

同様に、縦、横が2つずつスライドする場合はこのようになります。   

**(2,2)の場合**
![alt text](<スクリーンショット 2026-04-13 180154.png>)

このようにフィルターの移動をユーザーが設定できます。

## パディング

続いてパディングについてです。

下の図のように、入力画像の外側に、0の値を配置します。値が0の場合を特に **ゼロパディング** といいます。この手法を用いる理由は、基本的に畳み込み層やプーリング層では入力画像が小さくなってしまうからです。そのため層を深くするほど、画像の大きさが足りなくなってしまいます。それを補うために周りにデータを施すことで、画像を大きくしているのです。図の例では縦、横に一つずつ0の列を追加していますが、先ほどのストライドの設定のように、ユーザーが好きな列数を追加することができます。

![alt text](<スクリーンショット 2026-04-13 181123.png>)


これら二つのパラメーターを使用することで、出力されるデータのサイズを計算で求めることができます。
ストライドの移動するサイズを\\((stride_h,stride_w)\\)、パディングの追加するサイズを\\((pad_h,pad_w)\\)とするとこのような計算で出力サイズ\\((OH,OW)\\)が求められます。

$$OH = \frac{H + pad_h\cdot 2 - KH}{stride_h} + 1$$
$$OW = \frac{W + pad_w\cdot 2 - KW}{stride_w} + 1$$

ここでは入力画像とフィルターのサイズも使用します。こちらの変数に関しては下にまとめてあります。これらのパラメーターが決まることで、\\((OH,OW)\\)が求まり、**Conv2d** で用いることになるのです。
