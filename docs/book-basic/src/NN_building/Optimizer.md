# Optimizerの実装
では最後に **Optimizer** の実装です。Optimizerはパラメーターの更新を扱う構造体です。[誤差逆伝播法](./NN_syudou_jissou/gosa_gyakudenpan.md)のところでも触れましたが、パラメーターを更新する最適化関数の種類は様々あり、それを選択できるように様々な最適化を行う構造体を用意する必要があります。なので、こちらも今までと同様に、 **Optimizerトレイト** を実装し、このトレイトを実装した構造体を **Optimizer構造体** と呼ぶことにします。  
