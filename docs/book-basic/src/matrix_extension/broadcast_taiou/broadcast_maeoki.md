...
この場合、AまたはBをブロードキャストすることはできません。もちろん列数の2,3の最小公倍数を考えれば、AとBが同じような形状に拡張することはできるかもしれません。しかし、ブロードキャストは基本的に一方のサイズが1の行列の行または列を整数倍するのみなので、この場合は対応できず、エラーを吐きます。

---
では試しに②の例でArray型でブロードキャストを行ってみましょう。
...
このように、形状が異なる行列同士で足し算を行うことができます。これはaが自動で②の図のように変形したからです。
